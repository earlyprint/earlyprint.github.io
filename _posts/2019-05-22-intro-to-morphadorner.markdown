...
MorphAdorner was developed by scholars at Northwestern to meet the more specific purposes of processing a corpus such as EEBO-TCP, for which users are more likely to be scholars in the humanities with a range of tasks that include literary analysis and undergraduate- or graduate-level pedagogy.[^1] In the first place, MorphAdorner identifies its goals apart from other natural language processing tools:
...
[^1]:
     MorphAdorner’s website includes extensive documentation on the use of the tools as well as the history of their development: http://morphadorner.northwestern.edu/morphadorner/.
